# Nastenka AI - Your AI's Brain 🌑✨

> *"Context is a ghost until it finds a body. Nastenka is the vessel for your AI's Brain."*

**Nastenka AI** is a universal, model-agnostic **Persistent Cognitive Layer** designed to ensure your thoughts never die when you switch platforms. It is the "Resurrection of Context" for a multi-model world.

---

## 📖 The Story: Why "Nastenka"?

In Fyodor Dostoevsky's **"White Nights,"** Nastenka is the soulful witness to a dreamer's internal life. During the ethereal, sunlit nights of St. Petersburg, she listens to his dreams, his logic, and his vision. She is the one who remembers.

In the world of AI, we are all "Dreamers." We spend hours in deep flow with a model (Claude, GPT, Gemini), building complex **Cognitive Structures**. But when the token quota hits, or the context window saturates, we "wake up." We switch models, and suddenly, the new AI knows nothing. **Your AI's Brain** has been wiped clean. The context has died.

**Nastenka AI** is named after this witness. She sits above the models, ensuring that what you built in one dream carries over to the next—acting as a **Distributed Cognitive Shell** for your mind.

### ✝️ The Meaning: Anastasia (Anastasis)
The name **Nastenka** is a diminutive of **Anastasia**, derived from the Greek word ***Anástasis***, meaning **"Resurrection."** 

This is the technical mission of this project: **The Resurrection of your AI's Brain.** When a cognitive state "dies" in one platform, Nastenka rehydrates and resurrects that exact mental state in the next. 

---

## 🧠 The Core Philosophy: "One Cognitive State, Everywhere"

Current AI systems are **stateless**. They are locked in silos. **Nastenka AI** turns your interaction into a **Persistent Intelligence Layer**:

1.  **Identity Layer:** The unique behavioral fingerprint of your AI's brain.
2.  **Intent Layer:** Capturing the high-density cognitive goal of your current task.
3.  **Knowledge Layer:** A structured Graph of your past decisions and architectural "Truths."
4.  **Working Memory:** A portable cognitive state that transfers instantly between models.

---

## 🛠️ The Cognitive Architecture: The Neural Ledger

Built for the user who refuses to leave their brain behind.

*   **Tauri 2.0 (Rust):** The cross-platform host for your local brain (Mac, Win, Linux, Android, iOS).
*   **SurrealDB (Embedded):** A local-first **Knowledge Graph** database mapping the complex synapses of your thoughts.
*   **Automerge (CRDTs):** Ensures your "Mobile Brain" and "Desktop Brain" sync into a single cognitive continuum.
*   **Model Context Protocol (MCP):** The universal neural bridge that connects Nastenka to any AI provider.

---

## 🚀 The Vision: Ownership of Cognition

We believe the user should own their **Cognitive Architecture**, not the AI platform. Using **End-to-End Encryption (Curve25519)** and **Local-First** principles, Nastenka AI ensures that your brain's history is for your eyes only.

### **The Three Synaptic Tools:**
*   **`sync_current_intent`**: Captures the active pulse of your cognitive work.
*   **`get_architectural_grounding`**: Injects absolute mental truths into new models.
*   **`mark_structural_decision`**: Locks in your project's neural pathways to prevent hallucination.

---

## ⚖️ License

Distributed under the **MIT License**. See `LICENSE` for more information.

---

**"I am a dreamer; I have so little real life that I look upon such moments as this as so rare that I cannot help going over such moments again in my dreams."** 
— *Fyodor Dostoevsky, White Nights*

**Let's stop dreaming in silos. Let's start thinking in a Continuum.**
